...
def get_cqt_start_indexs(filename):
    y, sr = librosa.load(filename)
    rms = librosa.feature.rmse(y=y)[0]
    rms = [x / np.std(rms) for x in rms]
    ...
    for i in range(10,h):
        ...
        sum_before_col_cqt = np.sum([1 for i in range(len(before_col_cqt)) if before_col_cqt[i] == max_cqt])
        if sum_col_cqt >=5 and sum_before_col_cqt<=4:
            result.append(i)
