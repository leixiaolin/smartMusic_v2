...
if __name__ ==  '__main__':
    ...
    # filename = '../mp3/1.31WAV/节奏2.40分.wav'
    # clear_dir(image_dir)
    # wavname = filename
    # y, sr = librosa.load(filename)
    # rms = librosa.feature.rmse(y=y)[0]
    ...
    # # print(beats)
    # # beats_time = librosa.frames_to_time(beats,sr)
    # # librosa.display.waveplot(y, sr=sr)
    # # plt.vlines(beats_time, -1 * np.max(y), np.max(y), color='r', linestyle='solid')
    # # plt.show()
    #
    # # 获取一次判断的节拍帧和强度
    ...
    #     plt.ylabel('O')
    #     librosa.display.waveplot(y, sr=sr)
    #     plt.vlines(onset_frames_time, -1 * np.max(y), np.max(y), color='r', linestyle='solid')
    #
    #     plt.subplot(3, 1, 2)
    #     plt.ylabel('O-R')
    #     librosa.display.waveplot(y, sr=sr)
    #     plt.vlines(rms_onset_frames_time, -1 * np.max(y), np.max(y), color='r', linestyle='solid')
    #
    #     plt.subplot(3, 1, 3)
    #     plt.ylabel('O-R-M')
    #     librosa.display.waveplot(y, sr=sr)
    #     plt.vlines(pre_onset_time, -1 * np.max(y), np.max(y), color='r', linestyle='solid')
    #
    #     # plt.subplot(4, 1, 4)
    #     # plt.ylabel('O-M-R')
    #     # librosa.display.waveplot(y, sr=sr)
    ...
    for dir in dir_list:
        ...
        for filename in file_list:
            ...
            # total_frames = len(librosa.onset.onset_strength(y, sr))
            # range = total_frames/len()
            #plt.show()
            if onset_frames:
                onset_frames_time = librosa.frames_to_time(onset_frames, sr=sr)
